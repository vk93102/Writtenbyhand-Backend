...
curl -s -c /tmp/quiz_cookies.txt -X GET "http://127.0.0.1:8000/api/quiz/daily-quiz/?user_id=8&language=english" \
...
curl -s -b /tmp/quiz_cookies.txt -X POST "http://127.0.0.1:8000/api/quiz/daily-quiz/submit/" \
  -H "Authorization: Bearer $TOKEN" \
